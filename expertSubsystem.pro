...
clauses
    ...
    ask(_, [N | _]):- answer(N, нет), !, fail.

    % если признак присутствует в туре, то задается следующий вопрос
    ask(K, [N | Q]):- answer(N, да), !, ask(K, Q).

    ask(K, [N | Q]):- % задание вопроса о содержания признака в туре
        cond(N, C, Y),
        categ(C, X),
